...
$$
    ...
    &=\frac{\partial \log \left(\int h(x) \exp \{\eta^{T}T(x)\}T(x)dx\right) }{\int h(x) \exp \{\eta^{T}T(x) \}dx}\\
    &\overset{\eqref{eq:integral}}{=}\frac{\log \left(\int h(x) \exp \{\eta^{T}T(x)\}T(x)dx\right)}{\exp \{\eta^{T}T(x) \}}\\
    &=\log \left(\int h(x) \exp \{\eta^{T}T(x)\}dx\right)
    \end{aligned}
$$
