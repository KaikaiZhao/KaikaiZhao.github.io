...
From Eq. \eqref{eq:integral} and Eq. \eqref{eq:log-normalizer}, we can get why $$A(\eta)$$ is dubbed log normalizer.
 ...
 If we take the first derivative of the log normalizer, i.e. Eq. \eqref{eq:log-normalizer}, we have
 \begin{equation}
    \frac{\partial A(\eta)}{\partial \eta}=\log \left(\int h(x) \exp \\{T(x) \\}dx\right)
